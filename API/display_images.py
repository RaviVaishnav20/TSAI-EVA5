...
def find_custom_dataset_mean_std(DATA_PATH, cuda):
    ...
    dataloader_args = dict(shuffle=True, batch_size=256, num_workers=4, pin_memory=True) if cuda else dict(shuffle=True,
                                                                                                           batch_size=64)
...
def plot_images(images, PATH, name="fig", sub_folder_name="/visualization", is_cifar10=True, labels_list=None):
    ...
    for i in range(len(images)):
        ...
        if is_cifar10:
            CIFAR10_CLASS_LABELS = ["airplane", "automobile", "bird", "cat", "deer", "dog", "frog", "horse", "ship",
                                    "truck"]
            plt.title(
                f"{i + 1}) Ground Truth: {CIFAR10_CLASS_LABELS[images[i]['target']]},\n Prediction: {CIFAR10_CLASS_LABELS[images[i]['pred']]}")
        if labels_list is not None:
            plt.title(
                f"{i + 1}) Ground Truth: {labels_list[images[i]['target']]},\n Prediction: {labels_list[images[i]['pred']]}")
        else:
...
def show_save_misclassified_images(model, test_loader, device, PATH, name="fig", max_misclassified_imgs=25,
                                   is_cifar10=True, labels_list=None):
...
def show_save_correctly_classified_images(model, test_loader, device, PATH, name="fig",
                                          max_correctly_classified_images_imgs=25, is_cifar10=True, labels_list=None):
