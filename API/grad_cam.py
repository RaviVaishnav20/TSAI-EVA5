...
class GradCam(object):
    ...
    def __call__(self, data, target_layers, target_class=None):
        ...
        for target_layer in target_layers:
            ...
            saliency_map = F.interpolate(saliency_map, (self.img_h, self.img_w), mode="bilinear",
                                         align_corners=False)  # [64,1,32,32]
...
class VisualizeCam(object):
    ...
    def plot_heatmaps(self, img_data, target_class, img_name, nrows=2, ncols=5, figsize_height=10, figsize_width=4):
        fig, axs = plt.subplots(nrows=nrows, ncols=ncols, figsize=(figsize_height, figsize_width),
    ...
    def plot_img_heatmap(self, images, target_layers, PATH, target_inds=None, metric="", name="fig", columns=5,
                         figsize_height=10, figsize_width=15):
        pred_images = []
        ...
        fig, axs = plt.subplots(nrows=rows, ncols=columns, figsize=(figsize_width, figsize_height),
                                subplot_kw={'xticks': [], 'yticks': []})
        ...
        for ax, _ in zip(axs.flat, images):
            ...
            ax.set_title(
                f"{i + 1}) Ground Truth: {self.classes[images[i]['target']]},\n Prediction: {self.classes[images[i]['pred']]}",
                fontsize=8)
            i = i + 1
